...
export default function CreateOcView({ onCreated }: CreateOcViewProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const prompt = buildPrompt(description, selectedStyle, finalKeywords, lang);
...
const STYLE_ANCHORS_ZH: Record<string, string> = {
...
const STYLE_ANCHORS_EN: Record<string, string> = {
  pixel: 'Pixel art render: 32-bit game art, limited palette under 48 colors, visible pixel grid, clean color blocking, flat shadows without gradients, crisp outline, retro character-select screen quality.',
  anime: 'Japanese cel-animation style: clean precise linework, bright saturated color, soft lighting transitions and highlights, layered hair shadow blocks, expressive eyes, light-novel cover illustration quality.',
  cyber: 'Cyberpunk mech style: dark base with neon cyan, magenta, and amber accents, worn metal materials, mechanical parts, glowing circuit details, hard light and sci-fi concept art quality.',
  figure: '3D collectible figure render: PBR material feel, soft studio three-point lighting, miniature photography mood, subtle subsurface skin scattering, dimensional fabric folds, shallow-depth macro lens.',
  comic: 'Comic ink style: high-contrast ink lines, varied outline weight, cross-hatching shadows, selective accent color, dynamic cover composition, printed paper texture.',
  arcade: 'Retro arcade style: chunky pixel proportions, bright saturated high-contrast palette, simplified graphic shapes, minimal shadow layers, 1980s Japanese arcade character-select feel.',
};

function buildPrompt(description: string, styleId: string, keywords: string[], lang: 'zh' | 'en'): string {
  if (lang === 'en') {
    const keywordLine = keywords.length > 0
      ? `\nAppearance keywords: ${keywords.join(', ')}`
      : '';
    const styleAnchor = STYLE_ANCHORS_EN[styleId] || '';
    return `Pure white background, vertical 2:3 full-body character illustration, front or three-quarter standing pose, feet fully visible.

Character: ${description}${keywordLine}

Visual style: ${styleAnchor}

Composition and constraints:
- Single centered original character with headroom and footroom
- Clear clothing texture, accessory details, and layered hair
- Natural lively expression with focused eyes
- No text, watermark, signature, or UI elements
- Original character design; do not reference copyrighted characters`;
  }

  const keywordLine = keywords.length > 0
    ? `\n外貌关键特征：${keywords.join('、')}`
    : '';
  const styleAnchor = STYLE_ANCHORS_ZH[styleId] || '';
