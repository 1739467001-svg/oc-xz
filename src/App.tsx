...
function AppContent() {
  ...
  const [splashState, setSplashState] = useState(() => {
    return localStorage.getItem('zealwish.seenSplash') === '1' ? 'off' : 'on';
  });
  const [active, setActive] = useState<ViewId>('home');
  const [collapsed, setCollapsed] = useState(false);
  const [theme, setTheme] = useState('dark');
  const [ttsEnabled, setTtsEnabled] = useState(() => localStorage.getItem('zealwish.ttsEnabled') !== '0');
  const [isThinking, setIsThinking] = useState(false);
  const [sessions, setSessions] = useState<Record<string, Session>>(() => {
    const init: Record<string, Session> = {};
    const starter = createStarterSession(lang);
    init[starter.id] = starter;
    return init;
  });
  const [activeSessionId, setActiveSessionId] = useState('s1');
  const [avatarDataUrl, setAvatarDataUrl] = useState<string | undefined>(() => {
    return localStorage.getItem('zealwish.avatar') || undefined;
  });
  ...
  const dismissSplash = () => {
    ...
    localStorage.setItem('zealwish.seenSplash', '1');
    setTimeout(() => setSplashState('off'), 500);
  };

  const activeSession = sessions[activeSessionId];
  const sessionTitle = activeSessionId === 's1' && (activeSession?.messages.length || 0) === 0
    ? createStarterSession(lang).title
    : activeSession?.title;
  ...
  return (
    <GlassShell>
      ...
      {isMobile && <MobileTabBar active={active} setActive={setActiveView} />}
    </GlassShell>
  );
}

function createStarterSession(lang: 'zh' | 'en'): Session {
  return lang === 'en'
    ? { id: 's1', title: 'It got colder today. Bring an extra layer.', date: '14:32', preview: '"You went to bed at 1am last night."', messages: [] }
